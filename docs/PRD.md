...
- IT requests qua email/GitHub Issues trộn lẫn với dự án
...
- Leave/nghỉ phép không sync với GitHub Issues → manager không biết developer không available
...
- **Tích hợp native**: GitHub Issues, AMIS HRM, BambooHR, Google Workspace, Slack
...
- **Tech savvy**: Cao — đã dùng GitHub Issues, biết ServiceNow nhưng không có budget
...
### Epic 4: GitHub Issues Integration

**ST-009** — GitHub Issue Linking
> As an **IT agent**, I want to link a VietOps ticket to a GitHub issue so that developers and project managers have unified visibility.
- Acceptance: Search và link GitHub issue từ ticket detail
- GitHub issue status hiển thị trong VietOps ticket
- Comment sync (optional) giữa 2 hệ thống

**ST-010** — GitHub Issue Auto-create
> As an **IT Manager**, I want certain ticket types (e.g., infrastructure issues) to automatically create GitHub issues so that dev team is notified without manual steps.
- Acceptance: Config rule: ticket category + priority → auto-create GitHub issue in specified repository
- GitHub issue created với proper labels, description
...
- Checklist: laptop request, account creation (email/GitHub/Slack/VPN), mentor assignment
...
- Auto-create tasks: laptop return, disable email, disable GitHub access, revoke VPN
...
**ST-015** — Leave ↔ GitHub Issues Sync
> As a **Project Manager**, I want to see developer availability in GitHub Issues based on approved leave so that I don't assign work to people who are on leave.
- Acceptance: Approved leave triggers availability update
- GitHub Projects capacity dashboard shows actual available days
- Alert khi assigning issue to developer on leave
